...
{!!Form::close()!!}
	...
	<script>
		...
		function Addproducto(){
			...
			if (idarticulo !="" &&  descuento!="" && precio_venta!="") {
				...
				if (cantidad != "" && cantidad > 0) {
					...
					if (cantidad <= stock) {
						...
						var fila='<tr class="selected" id="fila'+contador+'"><td><button type="button" class="btn btn-warning" onclick="Eliminar('+contador+');">X</button></td> <td><input type="hidden" name="idarticulo[]" value="'+idarticulo+'">'+articulo+'</td> <td><input type="number" name="cantidad[]" value="'+cantidad+'"></td> <td><input type="number" name="precio_venta[]" value="'+precio_venta+'"></td> <td><input type="number" name="descuento[]" value="'+descuento+'"></td>  <td>'+subtotal[contador]+'</td> </tr>';
						contador++;
						Limpiarcajas();
						$('#total').html("S/.  "+total);
