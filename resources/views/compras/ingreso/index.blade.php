			...
			<div class="table-responsive">
				...
				@foreach($ingresos as $ing)
					<tr>
						...
						<td>{{ $ing->tipo_comprobante.' '.$ing.serie_comrpobante.' '.$ing.num_comprobante }}</td>
						<td>{{ $ing->impuesto }}</td>
						<td>{{ $ing->total }}</td>
						<td>{{ $ing->estado }}</td>
						<td>
							<a href="{{URL::action('IngresoController@edit',$ing->idingreso)}}"> <button class="btn btn-primary">Detalles</button> </a>
